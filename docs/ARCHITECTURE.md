...
앱은 비회원 사용을 기본 허용하고, 가족 프로필·즐겨찾기 동기화·복약 기록 보관처럼 서버 저장이 필요한 순간 회원가입을 유도합니다.

- 비회원: 앱 설치 시 생성한 `guest_id`로 로컬 최근 기록, 즐겨찾기, 제한적 가족 프로필을 저장합니다.
- 회원: Google/Kakao/Naver 소셜 로그인 후 서버가 사용자, 소셜 계정, 기기, refresh token을 저장합니다.
- 토큰: access token은 짧게 사용하고 refresh token은 기기별로 발급하며, 서버에는 원문 대신 hash를 저장합니다.
- 가족 프로필: MVP에서는 한 계정 안의 가족 프로필만 지원하고, 계정 간 공유 권한은 다음 단계로 미룹니다.
- 데이터 병합: 로그인 후 비회원 즐겨찾기, 최근 본 장소, 가족 프로필을 `/api/migration/guest-data`로 서버 저장소에 옮깁니다.
- 동의 관리: 약관, 개인정보, 위치, 푸시, 가족 건강 메모, 약 사진 기록, AI 개선, 마케팅 동의를 기능별로 분리합니다.
- 가족관리 강화: 현재 관리 대상을 앱 전역 기준으로 선택하고, 기본정보·건강정보·복약관리·응급카드·권한설정을 가족별로 관리합니다.
- 확장 모델: `family_groups`, `family_members`, `health_profiles`, `family_permissions`를 추가해 향후 초대/동의 기반 가족 연결과 권한 이력으로 확장합니다.
- 전역 가족 상태: `FamilyProfileProvider`가 현재 선택 가족을 제공하며 홈, 약국병원, 향후 복약·응급 화면이 같은 기준을 읽습니다.
- 병원·약국 연동: 자녀는 소아과/소아응급, 부모님은 내과/정형외과/야간진료를 우선하는 추천 칩과 정렬 점수를 적용합니다.
